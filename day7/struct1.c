#include <stdio.h>

//구조체 정의
struct menu {

	char* menu;        //메뉴의 이름
	int price;         //메뉴의 가격
};

int main1() {

	//구조체
	//프로그래밍 언어의 발전 : 업데이트(유지보수)
	
	//변수 : 차후 수정을 위해 값을 저장해 놓는 공간
	//배열 : 같은 주제의 변수들을 일열로 뭉쳐놓음(변수가 너무 많아지면 수정이 어려워짐)
	//함수 : 코드를 저장하는 공간(코드 재활용/재사용)
	//구조체 : 배열이 같은 자료형으로만 만들어지는 점을 극복(개발자가 직접 만드는 자료형)

	//카페 메뉴와 가격을 보관
	//(1) 변수
	//반복문 사용x, 코드찾기 오래 걸림, 코드 너무 길어짐, 관리 힘들어짐
	char* menu1 = "아메리카노", * menu2 = "에스프레소", * menu3 = "카페라테", * menu4 = "카페모카", * menu5 = "라떼";
	int price1 = 1500, price2 = 1000, price3 = 2500, price4 = 2500, price5 = 3000;

	//(2) 배열
	//같은 주제의 변수들을 뭉쳐놓음, 반복문 사용o, 코드찾기 오래걸림, 코드 너무 길어짐
	//무슨 주제로 묶였는지 까먹게됨, 짝궁을 잘 찾아줘야 함
	char* menu[5] = { "아메리카노","에스프레소","카페라테","카페모카","라떼" };
	int price[5] = { 1500,1000,2500,2500,3000 };

	//char*랑 int를 묶을 순 없을까? ===> 제공된 자료형으로 사용자가 다시 만드는 자료형(구조체)
	//(3)구조체
	struct menu coffee = { "아메리카노", 1500 };           //자료형 : struct menu,  변수명 : coffee
	struct menu cafe[5] = { {"아메리카노",1500},{"에스프레소",1000},{"카페라테",2500},{"카페모카",2500},{"라떼",3000}};

	printf("메뉴이름 : %s, 가격 : %d \n", coffee.menu, coffee.price);

	coffee.price = 2000;
	printf("메뉴이름 : %s, 가격 : %d \n", coffee.menu, coffee.price);








	return 0;
}